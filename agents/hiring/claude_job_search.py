"""
Agentic web-search loop for HiringPatternAgent.
...
def run_job_search(
    ...
    for iteration in range(MAX_SEARCH_ITERATIONS):
        ...
        limitations.append(
            f"Job search stopped at iteration {iteration}: {response.stop_reason}"
        )
        if text_output:
            return text_output, limitations, input_tokens, output_tokens
    ...
    limitations.append(
        f"Job search reached maximum iterations ({MAX_SEARCH_ITERATIONS})."
    )
    return "", limitations, input_tokens, output_tokens
