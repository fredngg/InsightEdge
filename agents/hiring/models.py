"""
Validation helpers for HiringPatternAgent evidence and score structures.
"""
...
def validate_evidence_item(item: dict, idx: int) -> dict:
    ...
    item.setdefault("id", f"hire_{idx + 1:03d}")
    item.setdefault("type", "software_engineering_growth")
    ...
    if item["type"] not in VALID_HIRING_CATEGORIES:
        item["type"] = "software_engineering_growth"
    ...
    # Low-confidence items with no URL must not be counted
...
def validate_score_breakdown(breakdown: dict, evidence: list[dict]) -> dict:
    """
    Clamp breakdown values to caps and zero out all categories
    if no counted evidence exists.
    """
