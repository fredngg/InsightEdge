"""
Agentic web-search loop for TechStackAgent.
...
from agents.tech_stack.config import MAX_SEARCH_ITERATIONS
...
def run_tech_search(
    ...
    for iteration in range(MAX_SEARCH_ITERATIONS):
        ...
        limitations.append(
            f"Tech search stopped at iteration {iteration}: {response.stop_reason}"
        )
        if text_output:
            return text_output, limitations, input_tokens, output_tokens
    ...
    limitations.append(
        f"Tech search reached maximum iterations ({MAX_SEARCH_ITERATIONS})."
    )
    return "", limitations, input_tokens, output_tokens
